...
export function Sidebar({ isOpen, toggleSidebar, userType }: SidebarProps) {
  ...
  // Lock body scroll when sidebar is open ONLY on mobile
  React.useEffect(() => {
    const isMobile = window.innerWidth < 768; // Define mobile as < 768px
    
    if (isOpen && isMobile) {
      // Prevent background scrolling when sidebar is open on mobile only
  ...
  return (
    <>
      ...
      <div
        ...
        } md:translate-x-0 overflow-y-auto z-50`}
      >
        <div className="p-6 flex flex-col min-h-full">
          ...
          <nav className="flex-1 flex flex-col justify-start space-y-1 mt-4 font-medium min-h-0">
            ...
            {/* Pending Requests - Only show for models */}
            {userType === "model" && (
              <>
                <hr className="border-t border-white/30" />
                <div className="py-3">
                  <NavItem
                    ...
                    to="/pending-requests"
                    isActive={location.pathname === "/pending-requests"}
                  />
                </div>
              </>
            )}
